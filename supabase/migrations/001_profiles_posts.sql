...
drop policy if exists "Профиль редактирует владелец" on public.profiles;
create policy "Профиль редактирует владелец"
  on public.profiles for all
  using (auth.uid() = id);

drop policy if exists "Посты читают все" on public.posts;
create policy "Посты читают все"
  on public.posts for select
  using (true);

drop policy if exists "Посты создаёт владелец" on public.posts;
create policy "Посты создаёт владелец"
  on public.posts for insert
  with check (auth.uid() = user_id);

drop policy if exists "Посты удаляет владелец" on public.posts;
