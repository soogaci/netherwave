...
import { PEOPLE, USER_PROFILES } from "@/lib/mock";
...
export default function ProfileUsernamePage() {
  ...
  const peoplePosts = PEOPLE.filter((p) => p.user === username);
  const isMe = username === "s1dead";

  const [following, setFollowing] = React.useState(false);
  const [followerCount, setFollowerCount] = React.useState(profile?.followers ?? 0);
