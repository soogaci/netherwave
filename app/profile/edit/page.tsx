...
export default function EditProfilePage() {
  ...
  const { user } = useAuth();
  const { profile, updateProfile, updateUsername, uploadAvatar } = useProfile();
  ...
  async function handleSave() {
    ...
    if (trimmedUsername && trimmedUsername !== profile?.username) {
      ...
      if (usernameError) {
        setSaving(false);
        toast?.(usernameError.message);
        return;
      }
    }
    const { error } = await updateProfile({
      username: trimmedUsername || profile?.username,
      display_name: displayName.trim() || trimmedUsername || profile?.username || "Пользователь",
      bio: bio.trim(),
      tags,
    });
    setSaving(false);
  ...
  async function handleAvatarChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file || !file.type.startsWith("image/")) {
      toast?.("Выбери изображение (jpg, png, gif)");
      return;
    }
      ...
      <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-5">
        <motion.div variants={item} className="flex justify-center">
          <label className="relative cursor-pointer block">
            ...
            />
            <div className="absolute -bottom-1 -right-1 grid h-7 w-7 place-items-center rounded-full bg-foreground text-background text-xs border-2 border-background pointer-events-none">
              {uploadingAvatar ? (
        ...
        <motion.div variants={item}>
          ...
          <div className="text-[10px] text-muted-foreground mt-1 px-1">Латинские буквы, цифры, подчёркивание. От 3 символов</div>
        </motion.div>
      </motion.div>
    </div>
  );
