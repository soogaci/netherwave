...
function HomeContent() {
  ...
  const { feed, subs, musicFeed, loading: feedLoading, refresh, toggleLike } = useFeed();
  ...
  React.useEffect(() => {
    if (feed.length > 0 || musicFeed.length > 0 || !feedLoading) setInitialLoading(false);
  }, [feed.length, musicFeed.length, feedLoading]);
  React.useEffect(() => {
        ...
        {initialLoading || feedLoading ? (
          ...
          <>
            {filteredFeed.map((item, i) => (
              <PeopleCard key={item.id} item={item} index={i} onLike={toggleLike} />
            ))}
            {filteredMusic.map((item) => (
              <MusicCard key={item.id} item={item} onLike={toggleLike} />
            ))}
          </>
        ) : (
    ...
    <>
      ...
      <section className="space-y-4">
        {initialLoading || feedLoading ? (
          ...
          <>
            {filteredSubs.map((p, i) => (
              <PeopleCard key={p.id} item={p} index={i} onLike={toggleLike} />
            ))}
            {filteredMusic.map((m) => (
              <MusicCard key={m.id} item={m} onLike={toggleLike} />
            ))}
          </>
        ) : (
  ...
  return (
    <div className="relative">
      ...
      <PageHeader
        left={
          <Link href="/add" className="p-1.5 rounded-full text-muted-foreground hover:text-foreground hover:bg-muted/60 transition" aria-label="Добавить пост">
